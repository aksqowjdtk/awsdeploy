...
# fabfile.py
# pip install fabric3
# fabric은 서버 세팅에 필요한 모든 과정을 파이썬으로 기록하여 원격 PC에서 자동으로 서버를 세팅하는 자동화 지원모듈이다
...
def setup():
    # 리눅스 업데이트 및 업그레이드
    _get_latest_apt()
    # 서비스 구동에 필요한 os 레벨상의 패키지를 설치해라
    _install_apt_requirements(apt_requirements)
    # 가상 환경을 사용할 수 있는 패키지를 설치해라
    _make_virtualenv()
...
# $> fab deploy
def deploy():
    # 저장서에 가서 최신 소스로 서버 상태를 반영한다(git 연동)
    _get_latest_source()
    # 생략
    #_put_envs()
    # 가상환경 업데이트
...
def _make_virtualenv():
    # 서비스 운영에 가상환경을 통해서 제공할것인데, 그 가상환경을 세팅하는 내용
...
def _get_latest_source():
    if exists(project_folder + '/.git'):
        ...
        # .git가 해당 프로젝트안에 없으면 저장소에 가서 프로젝트내용을 가져온다
...
def _update_virtualenv():
    ...
    if not exists(virtualenv_folder + '/bin/pip'):
        ...
        # 해당 가상환경안에 flask, scikit-learn이 2개가 설치 될것이다
